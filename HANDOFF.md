...
| Contact sheet choosing its source by which FILE exists | Preferring the delivered copy reviewed a regenerated texture as its own predecessor. Preferring the preview copy was wrong the other way: preview files are never cleaned up, so 98 renders from an earlier session shadowed the shipped textures and a sheet quietly mixed current and months-stale rows -- while carrying a "(UNVERIFIED PREVIEW)" label that read as routine | Decide from the SPEC, the same way the generator decides where to write: `visuallyVerified` true means the mod tree, false means the preview tree |
